...
import logo from "../assets/cloud-platform.png"
import { useAuthStore } from '../store';

const router = useRouter()
const store = useAuthStore()
const open = ref(false)
const { state, computedState } = storeToRefs(store)
const { clearAuth } = store

const navigation = [
    { name: 'Home', href: '/', current: true },
    // { name: 'Jobs', href: '/jobs', current: false },
    // { name: 'Add Job', href: '/jobs/create', current: false },
    { name: 'About', href: '/about', current: false },
    { name: 'Login', href: '/login', current: false },
    { name: 'Dashboard', href: '/dashboard', current: false },
]
// const user = computed(() => store.computedState)
// console.log(computedState.access_token);
    ...
    <Disclosure as="nav" class="bg-gray-800" v-slot="{ open }">
        ...
        <DisclosurePanel class="sm:hidden">
            <div class="space-y-1 px-2 pb-3 pt-2">
                <RouterLink class="flex" v-for="item in navigation" :key="item.name" :href="item.href" :to="item.href">
                    <DisclosureButton
                        :class="[item.current ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'rounded-md px-3 py-2 text-sm font-medium']"
                        :aria-current="item.current ? 'page' : undefined">
                        {{ item.name }}
